<?php
/**
 * Description of QuarterlyNewsletter
 *
 * @author tomborc
 */
class QuarterlyNewsletter implements Site {
    
    private ?array $data=[];
    private ?string $execute='setData';
    private ?array $prefix=[
        'pl'=>[
            'prefix'=>'/pl/informator-kwartalny/'
            ,'column'=>'title'
            ],
        'en'=>[
            'prefix'=>'/en/quarterly-newsletter/'
            ,'column'=>'titleEN'
            ]
        ];
    private ?object $URL;
    ...
    private function setData(){
        ...
        $result = $this->db->query("SELECT "
         ...
         . "WHERE "
         . "`n`.`status`=1 "
         . "AND `n`.`type`=1 ",MYSQLI_USE_RESULT);
