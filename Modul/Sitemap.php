...
class Sitemap {
   ...
   // }
    ...
    public function runDb(){
        //printf("%s\n","... ".__METHOD__."()");
		$this->Log->log(__METHOD__."()",0);
        $this->FTP->setConfig($this->ftpConfig['sql']);
        $this->FTP->connect();
        /*
        * Initialize Database
        */
        $dbProd=\Database::load($this->dbConfig['article'][$this->FTP->getFileContent('/db.txt')]);
        $dbAutos=\Database::load($this->dbConfig['autos']);
        $this->FTP->disconnect();
        /*
        * Producer
        */
        $Producer=new \Producer($dbAutos,new \URL());
        /*
         * Article
         */
        self::add(new \Article($dbProd,new \URL(),$Producer->getForProduct()));
        /*
         * Tires
         */
        self::add(new \Tire($dbProd,new \URL(),$Producer->getForProduct()));
        /*
         * Producer
         */
        self::add($Producer);
         ...
         * Promotion
         */
        self::add(new \Promotion($dbAutos,new \URL()));
         /*
         * News
         */
        self::add(new \News($dbAutos,new \URL()));
         /*
         * Traning
         */
        self::add(new \Training($dbAutos,new \URL()));
        /*
         * Guide
         */
        self::add(new \Guide($dbAutos,new \URL()));
        /*
         * Job (Work)
         */
        self::add(new \Job($dbAutos,new \URL()));
         /*
         * Gallery
         */
        self::add(new \Gallery($dbAutos,new \URL()));
        /*
         * QuarterlyNewsletter
         */
        self::add(new \QuarterlyNewsletter($dbAutos,new \URL()));
    ...
    public function upload():void{
        ...
        */
        $this->FTP->setConfig($this->ftpConfig['publisher']);
        $this->FTP->connect();
         /* remote dir autos.com.pl */
        $this->FTP->setWorkingDir($this->ftpConfig['publisher']['workingdir']);
        self::setOldSitemapFiles($this->FTP->getFileList());
        //var_dump(self::getOldSitemapFiles());
