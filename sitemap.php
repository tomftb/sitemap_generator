...
try{
    ...
    ($error!=='')? $Email->send('web@autos.com.pl',basename(__FILE__).' - the script execute failed',$error."<br/>".$warnings) : $Email->send('web@autos.com.pl',basename(__FILE__).' - the script was successful'.$overall_warning,'<p>Crawl site '.$argv[1].' execute in '.time()-$start.'s.</p><p>Execute `'.$argv[2].'` task.</p><p>'.$warnings."</p>");  
	$Email->close();
}
catch (Throwable $t){
    $Log->log($t->getMessage(),0);
